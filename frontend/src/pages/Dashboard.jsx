...
import { authAPI, storeAPI, teamAPI, removeAuthToken } from "../utils/api";
...
const Dashboard = () => {
  ...
  const fetchTeamData = async () => {
    try {
      const response = await teamAPI.myTeams();
      const firstTeam = response?.data?.teams?.[0]?.team;
      if (firstTeam) {
        setTeam(firstTeam);
      }
    } catch (err) {
      console.error("Failed to fetch team data:", err);
      // Don't set error here as team data is optional for stores page
    }
  };
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      ...
      <div className="h-12 border-b border-neutral-800 bg-neutral-900/50 backdrop-blur-sm flex items-center justify-between px-4 sticky top-0 z-50">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-md flex items-center justify-center font-bold text-white shadow-lg shadow-emerald-500/20">
              S
            </div>
          ...
          <span className="px-2.5 py-1 bg-neutral-800/80 text-neutral-300 rounded-md text-xs font-semibold border border-neutral-700/50">
            Free
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() =>
              window.open("https://discord.gg/ypFZ5MVS", "_blank")
            }
            className="px-3 py-1.5 text-neutral-300 hover:text-white hover:bg-neutral-800/60 rounded-md text-sm font-medium transition-all"
          >
            Feedback
          </button>
          ...
          <button
            ...
            className="w-8 h-8 rounded-md bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center font-bold text-white shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all"
          >
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6">
          ...
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold tracking-tight mb-1 bg-gradient-to-r from-white to-neutral-400 bg-clip-text text-transparent">
              Stores
            </h1>
            <p className="text-neutral-500 text-sm font-medium">
              Manage and monitor your stores
            </p>
          </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-2.5 rounded-lg border transition-all ${
                  viewMode === "grid"
                    ? "bg-neutral-800 border-neutral-700 text-emerald-400 shadow-sm"
                    : "border-neutral-800 text-neutral-400 hover:bg-neutral-800/50 hover:text-neutral-200"
                }`}
              >
                <svg
                  className="w-4 h-4"
          ...
          {showCreateForm && (
            <div className="mb-6 p-6 bg-neutral-900/50 border border-neutral-800 rounded-xl shadow-2xl">
              ...
              <p className="text-neutral-500 text-sm mb-5">
                Set up your new e-commerce project
              </p>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-neutral-100 group-hover:text-emerald-400 transition-colors">
                        {store.storeName}
                      </div>
                  ...
                  <div className="flex items-center gap-1 text-neutral-500 text-xs">
                    <div
                      className={`w-2 h-2 rounded-full ${
                        store.approved ? "bg-emerald-500" : "bg-amber-500"
                      }`}
                    ></div>
                    <span>
                      {store.approved
                ...
                <div
                  className="flex gap-2 pt-3 border-t border-neutral-800"
